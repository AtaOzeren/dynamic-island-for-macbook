...
This document specifies the AI agent state machine, the IPC protocol that carries agent state into KerNotch, the per-agent hook integrations for Claude Code, Codex CLI, and OpenCode, and the hook installer's consent model. `HookSnippetGenerator` is the source of truth for emitted configuration; Settings shows its current output when manual setup is required.

## Design principle

KerNotch is a status and control surface for AI coding agents, not an AI product itself. It runs no model, holds no API key, and never reads the screen to infer what an agent is doing. Every piece of AI status shown in the notch arrives as an explicit, agent-initiated message over the IPC protocol below. If an agent does not send a message, KerNotch shows nothing for it — there is no polling, no heuristic inference, and no fallback that guesses state from window titles or process activity.

## The agent state machine

`AIActivity` (a `KerNotchCore` activity, see `05-activity-model.md`) is driven by a state machine with seven states.
...
Entering `completed`, `waitingForUser` or `error` also lights the compact island in the badge's colour — green, yellow, red — for five passes — a two-second crossing, then a three-second rest — unless the user has switched the glow off in AI Integrations. Only the transition into the state lights it: an agent repeating the state it is already in does not restart the glow. A sub-agent finishing does not glow, because its instance is still working; a sub-agent asking or failing does, because it blocks the instance. The glow stops the moment its session moves on, a more urgent state takes it over, a less urgent one waits its turn, and the twenty-two seconds it lasts are the same twenty-two seconds a completed card stays on the island. See `docs/04-overlay-window.md` for how it is drawn.
...
- No other transition is legal. A message that requests an illegal transition (for example `idle` → `usingTool` with no preceding `thinking`) is **rejected** — the state machine returns a `.rejected` outcome and the state does not change. KerNotch does not crash or drop the connection, but the illegal state is never applied.

## The IPC protocol

The IPC protocol is a versioned contract between an agent (or its hook script) and KerNotch. Every message is a single JSON object called the **envelope**.

### Message envelope

| Field | Type | Required | Meaning |
|---|---|---|---|
| `schemaVersion` | string | yes | The envelope schema version this message conforms to (e.g. `"1.0"`). KerNotch rejects messages whose major version it does not understand. |
...
```json
{
  ...
  "title": "KerNotch AI Status Envelope",
  "type": "object",
...
KerNotch accepts the envelope over two transports. Both carry the identical JSON payload; the transport is purely a delivery mechanism.
...
| Agent | Failure signal | What KerNotch does |
|---|---|---|
...
| `completed` | 22 seconds | A display timeout, not a silence one: the task is over and the card is a receipt. It matches the attention glow's five passes, so the green light is never cut short by its own card leaving. |
...
- **Codex desktop app.** Its sessions run in app-server mode, where `notify` and lifecycle hooks are not delivered ([openai/codex#13019](https://github.com/openai/codex/issues/13019), [openai/codex#21639](https://github.com/openai/codex/issues/21639)). KerNotch shows nothing for those sessions until Codex delivers the events; Codex CLI sessions are unaffected.
...
KerNotch binds the HTTP listener to an OS-assigned ephemeral port at launch and writes the chosen port to a discoverable location (`~/Library/Application Support/KerNotch/ipc-port`) so a caller that prefers HTTP can read the current port without guessing or scanning.
...
- A message whose `agentId` is not one KerNotch recognizes, or one the user has not explicitly enabled in AI Integrations settings, is ignored — it does not create an activity, and it is not surfaced to the user as an error, since an unrecognized agent is expected to be either a future integration or an unrelated local process probing the port.

## Per-agent integration

Three agents are supported in V1: Claude Code, Codex CLI, and OpenCode. Each integrates through its own existing extension point — KerNotch adds no agent-side software beyond a small emitted snippet.

### Claude Code

Claude Code supports hook configuration in `~/.claude/settings.json`. KerNotch maps its lifecycle hooks onto agent state so a tool call cannot leave a permanently active card after the turn ends.

| Claude Code hook event | KerNotch state sent |
|---|---|
| `UserPromptSubmit` | `thinking` (detail "Task started") |
| `PreToolUse` | `usingTool` (detail "Using tool"; the session id is derived from the event JSON's `session_id` field). For `AskUserQuestion` and `ExitPlanMode` it sends `waitingForUser` instead (detail "Question asked" / "Plan approval needed") |
| `PostToolUse` | `working` (detail "Tool completed") |
| `PostToolUseFailure` | `working` (detail "Tool failed") |
| `PermissionRequest` | `waitingForUser` (detail "Needs attention", or the question tools' own detail) |
| `Notification` | `waitingForUser` (detail "Needs attention"); matcher limited to `permission_prompt`, `elicitation_dialog`, `elicitation_url_dialog`, `agent_needs_input`, `worker_permission_prompt` |
| `Stop` | `completed` (detail "Task completed") |
| `StopFailure` | `error` (detail "Task failed"; `reason` mapped from the event's `error` name) |
...
Claude Code hooks receive the event as JSON on stdin. The generated hook command is **asynchronous** — it backgrounds the loopback POST and returns immediately — so KerNotch's presence never adds latency to the agent's own execution.

The same hooks serve `claude` in a terminal and Claude Code sessions hosted by Claude Desktop, since both read `~/.claude/settings.json`. Profiles launched with their own `CLAUDE_CONFIG_DIR` read a different settings file and report nothing unless the hook is installed there too.

Only moments that genuinely need the user turn the card yellow. The question and plan-approval tools report `waitingForUser` from `PreToolUse` because their `usingTool` would be dropped by the default event switches, and `Notification` is narrowed to request-type notifications: unfiltered, `idle_prompt` (a minute after every finished turn) and `auth_success` each became a "needs your input" card for the full silence bound. A matcher made only of names is compared by Claude Code as an exact list. A rejected permission fires neither tool-result hook, so the next `PostToolUse` or the turn's `Stop` is what clears the card.

<!-- kernotch-snippet: claude-code -->
```json
{
  "hooks" : {
    "Notification" : [
    ...
    "PermissionRequest" : [
      {
        "hooks" : [
          {
            "command" : "KERNOTCH_PY=\"\"\nfor c in /opt/homebrew/bin/python3 /usr/local/bin/python3 \"$(command -v python3 2>/dev/null)\"; do\ncase \"$c\" in \"\"|/usr/bin/python3) continue;; esac\n[ -x \"$c\" ] && KERNOTCH_PY=\"$c\" && break\ndone\n[ -n \"$KERNOTCH_PY\" ] || { [ -d \"$(xcode-select -p 2>/dev/null)\" ] && KERNOTCH_PY=/usr/bin/python3; }\n[ -n \"$KERNOTCH_PY\" ] || exit 0\nEVENT=$(cat); { printf %s \"$EVENT\" | \"$KERNOTCH_PY\" -c 'import datetime, json, os, sys, urllib.request, uuid\n\nKERNOTCH_AGENT = \"claude-code\"\nkernotch_hook_v5 = True\n\n# A CLI launched by OpenCode reports as OpenCode, not as itself: the user\n# started one agent and expects one icon. OpenCode marks every child it\n# spawns, so the check is on the environment rather than on a process walk.\nif os.environ.get(\"OPENCODE\") or os.environ.get(\"OPENCODE_PID\"):\n    sys.exit(0)\n\n\ndef kernotch_load(raw):\n    try:\n        return json.loads(raw) if raw else json.load(sys.stdin)\n    except Exception:\n        sys.exit(0)\n\n\ndef kernotch_session(raw_session):\n    if not raw_session:\n        sys.exit(0)\n    return str(\n        uuid.uuid5(uuid.NAMESPACE_URL, KERNOTCH_AGENT + \":\" + str(raw_session))\n    )\n\n\ndef kernotch_payload(\n    session,\n    state,\n    detail,\n    tool_name=None,\n    root_session=None,\n    session_name=None,\n    workspace=None,\n    reason=None,\n):\n    payload = {\n        \"schemaVersion\": \"1.0\",\n        \"agentId\": KERNOTCH_AGENT,\n        \"sessionId\": session,\n        \"state\": state,\n        \"detail\": detail,\n        \"timestamp\": datetime.datetime.now(datetime.timezone.utc)\n        .isoformat(timespec=\"milliseconds\")\n        .replace(\"+00:00\", \"Z\"),\n    }\n    if state == \"usingTool\" and tool_name:\n        payload[\"toolName\"] = tool_name\n    if root_session:\n        payload[\"rootSessionId\"] = root_session\n    if session_name:\n        payload[\"sessionName\"] = session_name\n    if workspace:\n        payload[\"workspace\"] = workspace\n    if state == \"error\" and reason:\n        payload[\"reason\"] = reason\n    return json.dumps(payload, separators=(\",\", \":\"))\n\n\n# Claude Code names the cause in a closed set of its own. Mapped here\n# rather than passed through, because the island localises what it draws\n# and the validator refuses the punctuation a raw provider message is\n# full of.\nKERNOTCH_REASONS = {\n    \"rate_limit\": \"quotaExhausted\",\n    \"account_on_hold\": \"quotaExhausted\",\n    \"billing_error\": \"quotaExhausted\",\n    \"authentication_failed\": \"authFailed\",\n    \"oauth_org_not_allowed\": \"authFailed\",\n    \"overloaded\": \"providerUnavailable\",\n    \"server_error\": \"providerUnavailable\",\n    \"invalid_request\": \"requestRejected\",\n    \"model_not_found\": \"requestRejected\",\n    \"max_output_tokens\": \"requestRejected\",\n}\n\n\ndef kernotch_reason(event):\n    return KERNOTCH_REASONS.get(str(event.get(\"error\") or \"\"), \"unknown\")\n\n\ndef kernotch_tool_name(event):\n    raw = str(event.get(\"tool_name\") or \"\")\n    cleaned = \"\".join(c for c in raw if c.isalnum() or c in \" ._-\")[:80]\n    return cleaned or None\n\n\n# Deliver only to a running island. Missing or stale discovery data is\n# expected when KerNotch is not running, so failed events are dropped.\ndef kernotch_send(body):\n    try:\n        port = open(os.path.expanduser(\"~/Library/Application Support/KerNotch/ipc-port\")).read().strip()\n        if port:\n            urllib.request.urlopen(\n                urllib.request.Request(\n                    \"http://127.0.0.1:\" + port + \"/ai-status\",\n                    data=body.encode(\"utf-8\"),\n                    headers={\"Content-Type\": \"application/json\"},\n                ),\n                timeout=2,\n            )\n    except Exception:\n        pass\n\nKERNOTCH_USER_INPUT_TOOLS = {\n    \"AskUserQuestion\": \"Question asked\",\n    \"ExitPlanMode\": \"Plan approval needed\",\n}\n\nevent = kernotch_load(\"\")\nuser_input_detail = KERNOTCH_USER_INPUT_TOOLS.get(str(event.get(\"tool_name\") or \"\"))\nkernotch_send(\n    kernotch_payload(\n        kernotch_session(event.get(\"session_id\")),\n        \"waitingForUser\",\n        user_input_detail or \"Needs attention\",\n        None,\n        None,\n        None,\n        event.get(\"cwd\"),\n        None,\n    )\n)'; } >/dev/null 2>&1 &",
            "type" : "command"
          }
        ]
      }
    ...
    "PreToolUse" : [
      {
        "hooks" : [
          {
            "command" : "KERNOTCH_PY=\"\"\nfor c in /opt/homebrew/bin/python3 /usr/local/bin/python3 \"$(command -v python3 2>/dev/null)\"; do\ncase \"$c\" in \"\"|/usr/bin/python3) continue;; esac\n[ -x \"$c\" ] && KERNOTCH_PY=\"$c\" && break\ndone\n[ -n \"$KERNOTCH_PY\" ] || { [ -d \"$(xcode-select -p 2>/dev/null)\" ] && KERNOTCH_PY=/usr/bin/python3; }\n[ -n \"$KERNOTCH_PY\" ] || exit 0\nEVENT=$(cat); { printf %s \"$EVENT\" | \"$KERNOTCH_PY\" -c 'import datetime, json, os, sys, urllib.request, uuid\n\nKERNOTCH_AGENT = \"claude-code\"\nkernotch_hook_v5 = True\n\n# A CLI launched by OpenCode reports as OpenCode, not as itself: the user\n# started one agent and expects one icon. OpenCode marks every child it\n# spawns, so the check is on the environment rather than on a process walk.\nif os.environ.get(\"OPENCODE\") or os.environ.get(\"OPENCODE_PID\"):\n    sys.exit(0)\n\n\ndef kernotch_load(raw):\n    try:\n        return json.loads(raw) if raw else json.load(sys.stdin)\n    except Exception:\n        sys.exit(0)\n\n\ndef kernotch_session(raw_session):\n    if not raw_session:\n        sys.exit(0)\n    return str(\n        uuid.uuid5(uuid.NAMESPACE_URL, KERNOTCH_AGENT + \":\" + str(raw_session))\n    )\n\n\ndef kernotch_payload(\n    session,\n    state,\n    detail,\n    tool_name=None,\n    root_session=None,\n    session_name=None,\n    workspace=None,\n    reason=None,\n):\n    payload = {\n        \"schemaVersion\": \"1.0\",\n        \"agentId\": KERNOTCH_AGENT,\n        \"sessionId\": session,\n        \"state\": state,\n        \"detail\": detail,\n        \"timestamp\": datetime.datetime.now(datetime.timezone.utc)\n        .isoformat(timespec=\"milliseconds\")\n        .replace(\"+00:00\", \"Z\"),\n    }\n    if state == \"usingTool\" and tool_name:\n        payload[\"toolName\"] = tool_name\n    if root_session:\n        payload[\"rootSessionId\"] = root_session\n    if session_name:\n        payload[\"sessionName\"] = session_name\n    if workspace:\n        payload[\"workspace\"] = workspace\n    if state == \"error\" and reason:\n        payload[\"reason\"] = reason\n    return json.dumps(payload, separators=(\",\", \":\"))\n\n\n# Claude Code names the cause in a closed set of its own. Mapped here\n# rather than passed through, because the island localises what it draws\n# and the validator refuses the punctuation a raw provider message is\n# full of.\nKERNOTCH_REASONS = {\n    \"rate_limit\": \"quotaExhausted\",\n    \"account_on_hold\": \"quotaExhausted\",\n    \"billing_error\": \"quotaExhausted\",\n    \"authentication_failed\": \"authFailed\",\n    \"oauth_org_not_allowed\": \"authFailed\",\n    \"overloaded\": \"providerUnavailable\",\n    \"server_error\": \"providerUnavailable\",\n    \"invalid_request\": \"requestRejected\",\n    \"model_not_found\": \"requestRejected\",\n    \"max_output_tokens\": \"requestRejected\",\n}\n\n\ndef kernotch_reason(event):\n    return KERNOTCH_REASONS.get(str(event.get(\"error\") or \"\"), \"unknown\")\n\n\ndef kernotch_tool_name(event):\n    raw = str(event.get(\"tool_name\") or \"\")\n    cleaned = \"\".join(c for c in raw if c.isalnum() or c in \" ._-\")[:80]\n    return cleaned or None\n\n\n# Deliver only to a running island. Missing or stale discovery data is\n# expected when KerNotch is not running, so failed events are dropped.\ndef kernotch_send(body):\n    try:\n        port = open(os.path.expanduser(\"~/Library/Application Support/KerNotch/ipc-port\")).read().strip()\n        if port:\n            urllib.request.urlopen(\n                urllib.request.Request(\n                    \"http://127.0.0.1:\" + port + \"/ai-status\",\n                    data=body.encode(\"utf-8\"),\n                    headers={\"Content-Type\": \"application/json\"},\n                ),\n                timeout=2,\n            )\n    except Exception:\n        pass\n\nKERNOTCH_USER_INPUT_TOOLS = {\n    \"AskUserQuestion\": \"Question asked\",\n    \"ExitPlanMode\": \"Plan approval needed\",\n}\n\nevent = kernotch_load(\"\")\nuser_input_detail = KERNOTCH_USER_INPUT_TOOLS.get(str(event.get(\"tool_name\") or \"\"))\nkernotch_send(\n    kernotch_payload(\n        kernotch_session(event.get(\"session_id\")),\n        \"waitingForUser\" if user_input_detail else \"usingTool\",\n        user_input_detail or \"Using tool\",\n        kernotch_tool_name(event),\n        None,\n        None,\n        event.get(\"cwd\"),\n        None,\n    )\n)'; } >/dev/null 2>&1 &",
            "type" : "command"
          }
        ]
      }
...
Codex CLI supports one `notify` argv setting in `~/.codex/config.toml`. Codex spawns that program directly and appends event JSON as the final argument. KerNotch therefore emits a direct three-element Python command (`python3`, `-c`, script), without shell interpolation. The script reads Codex's appended argument, derives the session UUID from `thread-id`, and launches `open -g` detached.

The installer parses both single-line and multiline root `notify` arrays. If another notifier already exists, KerNotch preserves its argv and forwards every event to it before sending its own status. This is required for tools such as Codex Computer Use: installing KerNotch must not disable an existing notification integration. Original bytes are still backed up and restored on uninstall.
...
| Codex hook event | KerNotch state sent |
|---|---|
| `UserPromptSubmit` | `thinking` (detail "Task started") |
...
<!-- kernotch-snippet: codex -->
```toml
notify = ["/usr/bin/python3","-c","import datetime, json, os, sys, urllib.request, uuid\n\nKERNOTCH_AGENT = \"codex\"\nkernotch_hook_v5 = True\n\n# A CLI launched by OpenCode reports as OpenCode, not as itself: the user\n# started one agent and expects one icon. OpenCode marks every child it\n# spawns, so the check is on the environment rather than on a process walk.\nif os.environ.get(\"OPENCODE\") or os.environ.get(\"OPENCODE_PID\"):\n    sys.exit(0)\n\n\ndef kernotch_load(raw):\n    try:\n        return json.loads(raw) if raw else json.load(sys.stdin)\n    except Exception:\n        sys.exit(0)\n\n\ndef kernotch_session(raw_session):\n    if not raw_session:\n        sys.exit(0)\n    return str(\n        uuid.uuid5(uuid.NAMESPACE_URL, KERNOTCH_AGENT + \":\" + str(raw_session))\n    )\n\n\ndef kernotch_payload(\n    session,\n    state,\n    detail,\n    tool_name=None,\n    root_session=None,\n    session_name=None,\n    workspace=None,\n    reason=None,\n):\n    payload = {\n        \"schemaVersion\": \"1.0\",\n        \"agentId\": KERNOTCH_AGENT,\n        \"sessionId\": session,\n        \"state\": state,\n        \"detail\": detail,\n        \"timestamp\": datetime.datetime.now(datetime.timezone.utc)\n        .isoformat(timespec=\"milliseconds\")\n        .replace(\"+00:00\", \"Z\"),\n    }\n    if state == \"usingTool\" and tool_name:\n        payload[\"toolName\"] = tool_name\n    if root_session:\n        payload[\"rootSessionId\"] = root_session\n    if session_name:\n        payload[\"sessionName\"] = session_name\n    if workspace:\n        payload[\"workspace\"] = workspace\n    if state == \"error\" and reason:\n        payload[\"reason\"] = reason\n    return json.dumps(payload, separators=(\",\", \":\"))\n\n\n# Claude Code names the cause in a closed set of its own. Mapped here\n# rather than passed through, because the island localises what it draws\n# and the validator refuses the punctuation a raw provider message is\n# full of.\nKERNOTCH_REASONS = {\n    \"rate_limit\": \"quotaExhausted\",\n    \"account_on_hold\": \"quotaExhausted\",\n    \"billing_error\": \"quotaExhausted\",\n    \"authentication_failed\": \"authFailed\",\n    \"oauth_org_not_allowed\": \"authFailed\",\n    \"overloaded\": \"providerUnavailable\",\n    \"server_error\": \"providerUnavailable\",\n    \"invalid_request\": \"requestRejected\",\n    \"model_not_found\": \"requestRejected\",\n    \"max_output_tokens\": \"requestRejected\",\n}\n\n\ndef kernotch_reason(event):\n    return KERNOTCH_REASONS.get(str(event.get(\"error\") or \"\"), \"unknown\")\n\n\ndef kernotch_tool_name(event):\n    raw = str(event.get(\"tool_name\") or \"\")\n    cleaned = \"\".join(c for c in raw if c.isalnum() or c in \" ._-\")[:80]\n    return cleaned or None\n\n\n# Deliver only to a running island. Missing or stale discovery data is\n# expected when KerNotch is not running, so failed events are dropped.\ndef kernotch_send(body):\n    try:\n        port = open(os.path.expanduser(\"~/Library/Application Support/KerNotch/ipc-port\")).read().strip()\n        if port:\n            urllib.request.urlopen(\n                urllib.request.Request(\n                    \"http://127.0.0.1:\" + port + \"/ai-status\",\n                    data=body.encode(\"utf-8\"),\n                    headers={\"Content-Type\": \"application/json\"},\n                ),\n                timeout=2,\n            )\n    except Exception:\n        pass\n\nkernotch_codex_notify_v5 = True\nimport subprocess\nforward = json.loads(\"[]\")\nevent_args = sys.argv[1:]\nforward and subprocess.Popen(\n    forward + event_args,\n    start_new_session=True,\n    stdin=subprocess.DEVNULL,\n    stdout=subprocess.DEVNULL,\n    stderr=subprocess.DEVNULL,\n)\nif not event_args:\n    sys.exit(0)\nevent = kernotch_load(event_args[0])\nkernotch_send(\n    kernotch_payload(\n        kernotch_session(event.get(\"thread-id\") or event.get(\"thread_id\")),\n        \"completed\",\n        \"Turn completed\",\n    )\n)"]
```

### OpenCode

OpenCode uses a plugin file convention: a plugin file placed in OpenCode's plugin directory observes session lifecycle events and can act on them directly. KerNotch's installer writes a small TypeScript plugin that maps those events onto the same IPC envelope: `chat.message` becomes `thinking`, `tool.execute.before` becomes `usingTool`, `tool.execute.after` becomes `working`, `permission.asked` and `question.asked` (the question tool) become `waitingForUser`, `permission.replied`, `question.replied` and `question.rejected` become `working` so an answered prompt stops asking for the user, `session.idle` becomes `completed` — or `error` when the last assistant message carries a non-abort error — and `session.deleted` becomes `idle` (ending the presentation immediately, like the other agents' session-end events). The plugin derives a stable session UUID from the session id and posts to the loopback listener without awaiting it, so it never blocks OpenCode's own execution.
...
Installing or uninstalling also removes a plugin an earlier generation wrote — including the pre-rename `notchflow.ts`, which posts to a port file no island publishes any more. Such a file is recognised by its structure plus its transport, never by its name alone, and an earlier generation of KerNotch's own plugin is replaced without being backed up as if it were the user's.

The literal plugin file the installer writes, exactly as `HookSnippetGenerator` emits it:

<!-- kernotch-snippet: opencode -->
```ts
...
KerNotch writes an agent configuration only after that agent has been explicitly enabled or its install action accepted. The installer flow is:

1. **Detect.** On first run and on demand from Settings, KerNotch checks for the presence of `~/.claude/settings.json`, `~/.codex/config.toml`, and an OpenCode plugin directory, to determine which agents are installed on the machine.
2. **Propose.** For each detected agent, KerNotch shows the user the exact snippet it would add or change — not a summary, the literal text — before touching anything.
...
4. **Back up.** Before writing, KerNotch copies the original file alongside itself (e.g. `settings.json.kernotch-backup`) so the change is trivially reversible outside the app as well as through it.
