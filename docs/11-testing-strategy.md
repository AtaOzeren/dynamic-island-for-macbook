...
- **The `xcodebuild test` invocation.** Both schemes are tested independently: `xcodebuild test -scheme "KerNotch (App Store)" -destination "platform=macOS"` and the equivalent for `KerNotch (Direct)`, since the two configurations link different music providers (`10-build-and-distribution.md`) and a test that only runs against one scheme could miss a `Direct`-only or `AppStore`-only regression.
