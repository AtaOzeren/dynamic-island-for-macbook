...
When the user keeps the island pet (`docs/17-island-pet.md`), it lives on the compact island's leading flank, which then stays two places wide whatever the icons do: the icons fill it from the notch side, the pet roams while it is empty, sits in the outer place beside one icon, and runs out past the island's edge when two icons fill it. The pill's surface, its mask, its offset onto the notch, the glow and the hover target are all sized with the pet's flank. The pet is drawn behind the icons, only on the compact island, and only on the primary display. The hover peek's scale is undone for the pet (`islandHoverScale`): its art is drawn one image pixel per device pixel, and scaling it by the peek's 3% would resample it.
...
| Island pet | Walk 15 pt/s, run 30 pt/s, legs every 2/15 s walking and 1/15 s running; a roaming loop of about 21 s and a resting loop of about 16 s | Discrete Core Animation keyframes asking for 15 frames a second, only while the user has the pet switched on; see `docs/17-island-pet.md` |
