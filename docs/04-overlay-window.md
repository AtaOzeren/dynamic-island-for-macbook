...
When the user keeps the island pet (`docs/17-island-pet.md`), it lives on the compact island's leading flank, which then stays two places wide whatever the icons do: the icons fill it from the notch side, the pet roams while it is empty, sits in the outer place beside one icon, and runs out past the island's edge when two icons fill it. The pill's surface, its mask, its offset onto the notch, the glow and the hover target are all sized with the pet's flank. When the island opens, the pet moves onto the open island's strip beside the notch, above the cards, and back onto the flank when it closes: the root view draws the pet's stage itself, laid against the island's outer edge in both states, so the pet rides that edge on the island's own spring. It is drawn behind the icons and the cards, clipped by the island's mask like everything else, and only on the primary display. The hover peek's scale is undone for the pet (`islandHoverScale`): its art is drawn one image pixel per device pixel, and scaling it by the peek's 3% would resample it. The pet notices the pointer moving onto it through a tracking area rather than a hit test, so clicks still fall through it onto the island.
