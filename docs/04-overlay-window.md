...
When the user keeps the island pet (`docs/17-island-pet.md`), it has one place of its own at the outer end of the compact island's leading flank, and the icons take theirs beside it from the notch side: the pet roams its place while the flank has no icons, keeps to it beside one icon, and runs out past the island's edge when two icons fill the flank. It widens the pill by that one place, never more. The pill's surface, its mask, its offset onto the notch, the glow and the hover target are all sized with the pet's flank. When the island opens, the pet moves onto the open island's strip beside the notch, above the cards, and back onto the flank when it closes: the root view draws the pet's stage itself, laid against the island's outer edge in both states, so the pet rides that edge on the island's own spring. It is drawn behind the icons and the cards, clipped by the island's mask like everything else, and only on the primary display. The hover peek's scale is undone for the pet (`islandHoverScale`): its art is drawn one image pixel per device pixel, and scaling it by the peek's 3% would resample it. The pet notices the pointer moving onto it through a tracking area rather than a hit test, so clicks still fall through it onto the island.
...
| Island pet | Walk 15 pt/s, run 30 pt/s, legs every 2/15 s walking and 1/15 s running; a roaming loop of about 21 s and a resting loop of about 16 s; reactions of 1–4 s, hops a third of a second in the air | Discrete Core Animation keyframes asking for 15 frames a second, only while the user has the pet switched on; see `docs/17-island-pet.md` |
| Idle-state animation budget | Zero, apart from the pet the user switched on | No animation, timer-driven or otherwise, runs while the empty compact island is idle; this is part of the idle-cost contract from `docs/02-performance-contract.md`. The island pet is the one exception, off by default, played entirely by the render server |
...
- **Reduced motion.** When `NSWorkspace.shared.accessibilityDisplayShouldReduceMotion` is `true`, the spring animations in the table above are replaced with an instant or near-instant cross-fade; no transition is skipped outright, since the state still needs to visually change, but the motion component of it is removed. The attention glow fades in place on the same beat instead of travelling, the music slot shows its still note while a track plays, and the island pet sits still where its routine rests. KerNotch's own Motion setting in General ("Always reduce", "Never reduce") outranks the system setting for all of this — transitions and continuous motion alike, on every display — through the `prefersReducedIslandMotion` environment value every island view reads instead of `accessibilityReduceMotion`, which SwiftUI fills from the system alone. While the CPU watchdog has suspended motion, on every display, the glow is a still rim and nothing animates — the pet included.
