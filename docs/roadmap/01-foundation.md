...
**Unblocks:** Every later phase. Todo 17 specifically gates Wave 2 (Phase 2) — `KerNotchCore` and its test target must exist before any TDD work can start. The plan's dependency matrix marks this "2-wide after todo 17": once the skeleton exists, the remaining six todos have no ordering constraint on each other.
...
Create an Xcode project for a macOS app named `KerNotch`, deployment target macOS 14.0, Swift 6 language mode with strict concurrency. Create a local Swift package containing three library targets — `KerNotchCore`, `KerNotchProviders`, `KerNotchUI` — plus three matching test targets. `KerNotchCore` must declare no dependencies beyond Foundation. Wire the app target to depend on all three. Add `.gitignore` for Xcode/SPM artifacts.

- **Acceptance:** `xcodebuild -scheme KerNotch build` succeeds; `swift test` runs (zero tests is acceptable at this point); `KerNotchCore` compiles without importing AppKit.
- **QA (CI):** Run the build and the test command; capture both exit codes. Evidence: `.omo/evidence/task-17-kernotch-v1.log`.
...
- **QA (CI):** `swiftlint --strict` and the format check both exit zero. Evidence: `.omo/evidence/task-19-kernotch-v1.log`.
- **Commit:** `build: add lint and format configuration`

### 20. Add the architecture guard script

A script that fails if `KerNotchCore` sources import AppKit, SwiftUI, or any provider module — enforcing the dependency rule from `docs/01`.
...
- **QA (HW):** Launch, observe no Dock icon, toggle the setting twice, and read back the service status each time. Evidence: screenshots plus status output at `.omo/evidence/task-23-kernotch-v1/`.
