...
- **QA (CI + HW):** Unit tests for parsing; on hardware, invoke the scheme for each of the seven states and one malformed case. Evidence: `.omo/evidence/task-50-kernotch-v1.log`.
...
- **QA (HW):** Confirm the listening socket's bind address; attempt a connection from a non-loopback address and assert refusal; disable all integrations and assert the socket is gone. Evidence: `.omo/evidence/task-51-kernotch-v1.log`.
...
- **QA (CI):** Structural tests per state; on hardware, drive all seven states and screenshot each. Evidence: `.omo/evidence/task-52-kernotch-v1/`.
...
- **QA (HW):** Install, diff, run a Claude Code session and observe states in the island, uninstall, and diff against the original. Evidence: `.omo/evidence/task-54-kernotch-v1/`.
...
- **QA (HW):** As todo 54, with Codex. Evidence: `.omo/evidence/task-55-kernotch-v1/`.
...
- **QA (HW):** As todo 54, with OpenCode. Evidence: `.omo/evidence/task-56-kernotch-v1/`.
...
- **QA (CI):** Assert the UI snippet and the installer output are produced by the same generator and are string-equal. Evidence: `.omo/evidence/task-57-kernotch-v1.log`.
...
- **QA (HW):** Disable one event class, emit it, and assert no activity is created. Evidence: `.omo/evidence/task-58-kernotch-v1.log`.
...
- `.omo/plans/kernotch-v1.md` — Wave 5 (lines 403-457), dependency matrix (lines 84-95).
