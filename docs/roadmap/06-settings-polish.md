...
**Status:** NOT STARTED
**Todos:** 59–65 (Wave 6 in `.omo/plans/kernotch-v1.md`)
...
- **QA (CI):** Tests over a clean defaults domain asserting each default. Evidence: `.omo/evidence/task-59-kernotch-v1.log`.
...
- **QA (HW):** Walk every pane and exercise every control; screenshot each pane. Evidence: `.omo/evidence/task-60-kernotch-v1/`.
...
- **QA (HW):** Reset the defaults domain and launch; confirm the flow and that no permission prompt appears unless a feature is enabled. Evidence: `.omo/evidence/task-61-kernotch-v1/`.
...
- **QA (CI):** `swiftlint --strict` passes with the custom rule enabled. Evidence: `.omo/evidence/task-62-kernotch-v1.log`.
...
- **QA (CI + HW):** A script asserts zero missing translations; on hardware, run in Turkish and screenshot every surface. Evidence: `.omo/evidence/task-63-kernotch-v1/`.
...
- **QA (HW):** Fresh install, launch, confirm no prompts; enable each feature in turn and confirm one explained prompt each; deny each and confirm graceful degradation. Evidence: `.omo/evidence/task-64-kernotch-v1/`.
...
- **QA (CI + HW):** Asset validation in the build; screenshots of the status item in both appearances. Evidence: `.omo/evidence/task-65-kernotch-v1/`.
...
- `.omo/plans/kernotch-v1.md` — Wave 6 (lines 459–501), dependency matrix (lines 84–95).
