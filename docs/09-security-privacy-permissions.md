...
KerNotch persists only user preferences (see `08-settings-and-localization.md`): which providers are enabled, which AI agents are enabled, window and display choices, and language selection. The Discord integration additionally stores its OAuth2 access and refresh tokens as one generic-password Keychain item per Client ID (service `com.kernotch.KerNotch.discord`), deleted when the user disconnects or Discord refuses to renew them. KerNotch keeps no history of past activities, no record of Discord channels, and no log of AI agent content — once an `AIActivity` reaches `completed` or `error` and is dismissed, nothing about its detail text or tool names is retained anywhere on disk.
