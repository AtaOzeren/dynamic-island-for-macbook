...
- [ ] Copy `Casks/kernotch.rb` to the local `Homebrew/homebrew-cask` checkout at
      `Casks/k/kernotch.rb`.
...
brew style --fix Casks/k/kernotch.rb
brew audit --cask --new --online kernotch
brew install --cask kernotch
open -a KerNotch
brew uninstall --cask kernotch
brew install --cask kernotch
brew uninstall --cask --zap kernotch
```

- [ ] The style and audit commands finish without offenses or errors.
- [ ] Installation copies `KerNotch.app` to `/Applications`.
...
- [ ] `--zap` removes only KerNotch's preferences and application-support data.
- [ ] Save the complete command output as
      `.omo/evidence/task-71-kernotch-v1.log` for the plan's QA evidence.

## Submit upstream

- [ ] Create a branch in a personal fork of `Homebrew/homebrew-cask` and commit
      only `Casks/k/kernotch.rb`.
