...
- **Event source:** The system's own screen recording — the one started from the screenshot toolbar. Three signals, none of which needs a permission: `NSWorkspace` reports the capture UI coming and going, a file-system watch on `~/Library/Group Containers/group.com.apple.screencapture/ScreenRecordings` reports the movie appearing, and ReplayKit's open recording movie says whether a recording is actually under way.
- **Exact API:** `NSWorkspace.didLaunchApplicationNotification` / `didTerminateApplicationNotification` and the `runningApplications` key path for the capture UI; `DispatchSource.makeFileSystemObjectSource` on the recordings folder; `proc_listpids`/`proc_pidfdinfo` to see whether `replayd` holds a `.mov` open in that folder. KerNotch deliberately does **not** query `ScreenCaptureKit` session state: that is behind the Screen Recording permission, and `09-security-privacy-permissions.md` refuses to ask for the very capability the indicator reports on.
- **Permission or entitlement:** None.
...
- **What decides:** only the probe, which answers yes when `replayd` holds the movie open **or** the recordings folder holds a movie written within the last few seconds. The second reading exists because the first is late: `replayd` opens the movie a beat after recording starts, and the indicator arrived visibly after the recording did. Freshness rather than mere existence, so a movie ever left behind clears itself within seconds. The capture UI quits once recording is under way, so its absence says nothing at all; an observer that read it as "not recording" showed nothing for the whole session.
- **Update cadence:** The probe is read when observation starts — so a recording already under way when KerNotch launches is seen — when the capture UI comes or goes, and when the recordings folder changes. A low-frequency tick keeps reading it while a recording is live, while the toolbar is open, and for ten seconds after the folder changes, which is what catches a recording that is still getting under way. Otherwise the tick is cancelled: an idle island keeps no timer, per `02-performance-contract.md`.
- **Teardown:** `end()` fires when the probe reports the recording movie closed.
- **CI-vs-hardware verifiability:** A real recording can only be made on a Mac, and the probe reads live processes; the gating, the session latch, the start-up reading and the folder watch are unit-tested in CI against fakes and a temporary directory.
- **Honest statement of detectability:** KerNotch shows the system's own screen recording, not *which app* is recording — and a third-party recorder that leaves no distinguishable trace is not detected at all. No indicator is shown rather than a guessed one.
