...
- **Interaction with Audio Recording:** while the integration is on, `SystemAudioRecordingObserver` leaves Discord out; its session ends only when every microphone client is known and excluded, so another application sharing the microphone keeps the ordinary indicator on screen beside the call. A client is a process running input *on an input device* (`kAudioProcessPropertyDevices`, read in the input scope — the global scope answers empty): on macOS 26 `corespeechd`, listening for "Siri", reports running input for minutes with no input device, and counting it put the ordinary indicator beside every call.
