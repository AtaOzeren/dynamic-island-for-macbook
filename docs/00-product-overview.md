...
- KerNotch does not run or host any AI model itself. It only displays status pushed to it by AI agent hook scripts.
- KerNotch does not use Accessibility APIs or screen capture to observe other apps. It relies on system frameworks and scripting interfaces only.
- KerNotch is not a general-purpose menu bar replacement or system monitor.
- KerNotch does not attempt to be a public developer platform in V1. A public API and SDK are explicitly future work.

## V1 Feature List (User-Visible Capabilities)

The first release includes:

- **Music now-playing and transport control** — see the current track from Spotify and Apple Music (and, on macOS releases before 15.4, any app that reports now playing to the system, such as YouTube Music), with play/pause and next/previous controls, in both a compact and an expanded view.
...
- **Third-party download progress** — showing another app's download progress would require inspecting that app's internals, which is not available through public APIs.
...
- **Third-party developer API and SDK, including a `KerNotch.show(...)`-style public interface** (V2) — opening the platform to outside developers is a governance and stability commitment that comes after the core product has shipped and settled.

## Success Criteria

KerNotch's V1 is successful when a user can:
...
- Install the app from the notarized DMG or with Homebrew and get the same experience either way, since both deliver the same build.
