...
The island pet (`docs/17-island-pet.md`) is the one thing that moves on an idle island, and only because the user switched it on — it is off by default. The budget above is measured with it off. With it on, KerNotch itself must still measure as idle: the pet's routine — its reactions and its nap included — is played by the render server, so its cost lands there, at the 15 frames a second the pet asks for, and never in KerNotch's own CPU or wakeups.
...
| Continuous motion that *is* a running activity's status — the agent's working dot, the equaliser while a track plays, the bounded attention glow — runs as a Core Animation layer animation, re-armed only when its view returns to a window, and stops with the activity, under Reduce Motion, and while the watchdog has suspended motion | A per-frame SwiftUI animation re-evaluates the island every frame (measured at ~11% CPU per panel); a layer animation runs in the render server with the view evaluated once |
| Clock-driven presentation changes — a paused track's note leaving, a glow ending, a blocked agent's announcement expiring — share one wake-up scheduled for the soonest deadline, and none is scheduled when nothing is counting down | One sleeping task per island instead of a timer per feature, and no wake-up at all while idle |
| The island pet's routine is decided when something changes — its stage, its mood, or news worth a reaction, all found on refreshes the island already makes — and handed to Core Animation whole, the nap an empty island ends in included: discrete keyframe tracks for the pet and each effect beside it, asking for 15 frames a second (at most 30), with no timer, task or SwiftUI animation behind it, and nothing attached while it is off the island | The pet is the only motion on an idle island; played in the render server it costs KerNotch no wake-ups, and the low frame rate keeps a ProMotion display from running at 120 Hz for it |
