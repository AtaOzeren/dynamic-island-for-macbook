...
The island pet (`docs/17-island-pet.md`) is the one thing that moves on an idle island, and only because the user switched it on — it is off by default. The budget above is measured with it off. With it on, KerNotch itself must still measure as idle: the pet's routine is played by the render server, so its cost lands there, at the 15 frames a second the pet asks for, and never in KerNotch's own CPU or wakeups.
...
| The island pet's routine is decided when its stage changes and handed to Core Animation whole: discrete keyframe tracks asking for 15 frames a second (at most 30), with no timer, task or SwiftUI animation behind it, and nothing attached while it is off the island or the island is expanded | The pet is the only motion on an idle island; played in the render server it costs KerNotch no wake-ups, and the low frame rate keeps a ProMotion display from running at 120 Hz for it |
