...
KerNotch ships from one codebase in two forms: a sandboxed Mac App Store build and a Developer ID signed, notarized build for direct download and Homebrew. This document specifies how the split is implemented, the release pipeline for each channel, and which steps cannot proceed until the Apple Developer Program membership is purchased. It is a design specification, nothing in this folder is code.
...
Both configurations build from the same `KerNotchCore`, `KerNotchProviders`, and `KerNotchUI` targets described in `01-architecture.md`. Only the provider chosen at compile time and the entitlements file differ.
...
- **Swift compilation conditions.** Each configuration defines a matching active compilation condition (`APPSTORE_BUILD` or `DIRECT_BUILD`). `KerNotchProviders` uses this to select the music provider implementation at compile time, not at runtime, so the App Store binary never links the MediaRemote symbol table at all.
- **Per-configuration entitlements files.** `KerNotch-AppStore.entitlements` and `KerNotch-Direct.entitlements`, each listing only the entitlements that configuration needs, per the table in `09-security-privacy-permissions.md`.
- **Separate schemes.** `KerNotch (App Store)` and `KerNotch (Direct)`, each pinned to its configuration, its entitlements file, and its signing identity, so `xcodebuild -scheme "KerNotch (App Store)"` and `xcodebuild -scheme "KerNotch (Direct)"` are the two build entry points CI and local development both use.
...
1. **Bundle identifier.** Reverse-DNS under the developer's own domain, for example `dev.<author>.kernotch`. Never contains "dynamicisland" or "macbook", per the naming rule in `14-glossary-and-conventions.md`.
...
5. **Privacy nutrition label.** Answered directly from the data collection statement in `09-security-privacy-permissions.md`: no data collected, no tracking, since KerNotch has no analytics and no network calls beyond the loopback listener.
...
```bash
xcrun notarytool submit KerNotch.zip \
  --keychain-profile "kernotch-notary" \
  --wait
```

4. **Stapling.** On success, the notarization ticket is attached to the app bundle with `xcrun stapler staple KerNotch.app`, so the app opens offline without a Gatekeeper network check.
5. **`.dmg` layout.** A disk image containing `KerNotch.app` and an `Applications` symlink, background image optional, built with `create-dmg` or a hand-rolled `hdiutil` script, then itself notarized and stapled.
...
cask "kernotch" do
  ...
  url "https://github.com/<owner>/kernotch/releases/download/v#{version}/KerNotch-#{version}.dmg"
  name "KerNotch"
  desc "Turns the MacBook notch into a live-activity island"
  homepage "https://github.com/<owner>/kernotch"
  ...
  zap trash: [
    "~/Library/Preferences/dev.<author>.kernotch.plist",
