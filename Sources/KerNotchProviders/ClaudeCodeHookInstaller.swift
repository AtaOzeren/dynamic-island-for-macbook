...
public struct ClaudeCodeHookInstaller: Sendable {
    ...
    public init(
        ...
        legacyBackupURL = settingsURL.appendingPathExtension(
    ...
    private func mergeGeneratedHooks(
        ...
        for (event, generatedValue) in generatedHooks {
            ...
            // Drop the commands an earlier KerNotch wrote before adding this
    ...
    /// Whether this hook group is one KerNotch wrote, in any version.
    ///
    ...
    private func isManagedGroup(_ group: [String: Any]) -> Bool {
        ...
        return handlers.contains { handler in
            ...
            // Every token of a set, any set: see `legacyManagedHookMarkerSets`.
            return HookSnippetGenerator.legacyManagedHookMarkerSets.contains { markers in
                markers.allSatisfy(command.contains)
            }
        }
    }
