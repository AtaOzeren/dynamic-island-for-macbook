...
public struct CodexHookInstaller: Sendable {
    ...
    /// The suffix backups carried before the app was renamed. Read, never
    /// written, so uninstalling still restores a backup an earlier version left.
    ...
    public init(
        ...
        legacyBackupURL = configURL.appendingPathExtension(
            String(Self.legacyBackupSuffix.dropFirst())
        )
    }
    ...
    private func uninstallNotify() throws {
        if let (url, backup) = try existingBackup() {
    ...
    private func configurationBySettingRootNotify(in existing: String) throws -> String {
        if let assignment = try rootNotifyAssignment(in: existing) {
            ...
            // tool's to own, so only KerNotch's element inside it is touched —
            // and only when it is a previous version's. Left alone, that
            // element kept the launch fallback it was written with: every turn
            // Codex reported through the wrapper relaunched a quit KerNotch.
            ...
            // A `notify` an earlier KerNotch wrote is replaced rather than
    ...
    /// Whether these arguments are a `notify` command an *earlier* KerNotch
    ...
    private func isLegacyManagedNotify(_ arguments: [String]) -> Bool {
        ...
        let schemes = [HookSnippetGenerator.urlScheme, HookSnippetGenerator.legacyURLScheme]
        guard schemes.contains(where: { command.contains("\($0)://ai-status") }) else {
            return false
        }
        return command.contains(#"agentId":"codex"#)
            || schemes.contains { command.contains("\($0)_codex_notify_") }
    }
    ...
    private func forwardedArguments(from arguments: [String]) -> [String]? {
        ...
        let marker = "kernotch_forward_b64='"
        guard
    ...
    /// The chain with every nested KerNotch notify an earlier version wrote
