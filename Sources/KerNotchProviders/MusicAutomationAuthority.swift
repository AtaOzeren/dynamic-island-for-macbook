...
/// everything KerNotch decides on top of that answer.
///
...
/// member that can, so every prompt in KerNotch has exactly one caller to audit.
@MainActor
public protocol MusicAutomationAuthorizing: AnyObject {
    ...
    /// changed in System Settings while KerNotch was inactive.
...
public extension Notification.Name {
    static let musicAutomationPermissionDidChange = Notification.Name(
        "com.kernotch.music-automation-permission-did-change"
    )
}
...
public final class AppleEventsAutomationAuthority: MusicAutomationAuthorizing {
    ...
    private let permissionQueue = DispatchQueue(
        label: "com.kernotch.music-automation-permission",
        qos: .userInitiated
    )
...
///    sent an event until `explainer` has shown KerNotch's own account of what
...
public final class MusicAutomationGate {
    /// Shows KerNotch's plain-language explanation and answers whether the user
