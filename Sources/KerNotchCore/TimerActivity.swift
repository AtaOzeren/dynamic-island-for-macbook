...
/// Which direction KerNotch's own clock runs.
///
...
/// KerNotch's own countdown or stopwatch — the one V1 activity the app is both
...
/// in `KerNotchProviders` is what re-derives it on each tick. Nothing here
...
public struct TimerActivity: Activity, Equatable {
    ...
    public static let identity = ActivityIdentity("kernotch.timer")
