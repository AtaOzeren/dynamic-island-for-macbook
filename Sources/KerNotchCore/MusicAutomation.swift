...
/// state where a prompt is still available and KerNotch owes the user an
...
/// forbidden, because the flow says KerNotch "does not nag, re-prompt on a
...
/// What KerNotch may currently do with one music app, derived from that app's
/// permission status.
///
...
public struct MusicAutomationAccess: Equatable, Sendable {
    ...
    /// The plain-language explanation shown in KerNotch's own UI *before* the
    ...
    public var explanation: String {
        ...
        case .notDetermined:
            localized(
                """
                KerNotch will ask macOS for permission to control \(target.displayName), \
                so the notch can show what is playing and its buttons can control playback. \
                Nothing is sent anywhere; KerNotch only talks to \(target.displayName) on this Mac.
                """)
        case .granted:
            localized("KerNotch can show and control what \(target.displayName) is playing.")
        case .denied:
            localized(
                """
                \(target.displayName) control is off because macOS permission was declined. \
                Everything else in KerNotch keeps working. \
                To turn it on, allow KerNotch for \(target.displayName) in \
