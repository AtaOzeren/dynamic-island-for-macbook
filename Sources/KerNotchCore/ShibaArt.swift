...
extension ShibaArt {
    /// One pixel of art, placed over a frame: how a blink or a bark is drawn on
    /// a pose without copying the whole frame. The transparent key erases.
    struct ArtPixel {
        let column: Int
        let row: Int
        let key: Character
    ...
    static func loweringHead(_ rows: [String]) -> [String] {
        let headColumns = 17..<32
        var grid = rows.map(Array.init)
        let head = grid.prefix(12).map { Array($0[headColumns]) }
        for row in 0..<12 {
        ...
        for (row, line) in head.enumerated() {
            for (offset, key) in line.enumerated() where key != PetSpriteSheet.transparentKey {
                grid[row + 1][headColumns.lowerBound + offset] = key
            }
        }
        return grid.map { String($0) }
    }
    ...
    static func emptyRows(_ count: Int) -> [String] {
        Array(repeating: String(repeating: PetSpriteSheet.transparentKey, count: 32), count: count)
    }
}
