...
/// Whether an agent's configuration file currently carries KerNotch's hook.
///
/// Deliberately distinct from `KerNotchProviders.AgentInstallationStatus`,
...
/// settings pane that renders it is in `KerNotchUI`, which does not depend on
/// `KerNotchProviders`. Core is the one module both sides already share, so
