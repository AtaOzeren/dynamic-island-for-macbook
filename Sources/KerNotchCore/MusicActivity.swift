...
/// `MusicProvider` in `KerNotchProviders` is what actually sends one.
...
/// vocabulary above `KerNotchProviders`.
///
...
public struct MusicActivity: Activity, Equatable {
    ...
    public static let identity = ActivityIdentity("kernotch.music")
    ...
    public var primaryAction: PrimaryAction? {
        ...
        return PrimaryAction(
            title: localized("Open \(source)"),
            symbolName: "arrow.up.forward",
            intent: .openApplicationNamed(source)
        )
    }
}
