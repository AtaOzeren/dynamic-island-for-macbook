...
## [2026-08-30] Task: F1 fresh-checkout blockers
SwiftPM scans every item under an explicit target path, including resources owned only by the Xcode app target, so `NotchFlow/Assets.xcassets` and `NotchFlow/Localizable.xcstrings` belong beside `Info.plist` in the executable target's `exclude` list. Xcode scheme post-actions also run after `clean`; binary scanners must therefore exit successfully when `${TARGET_BUILD_DIR}/${EXECUTABLE_PATH}` is not executable, while still scanning normal build products. The music-backend guard stays fast and deterministic with one ignored, script-local DerivedData cache, unsigned quiet incremental builds, and an executable-product check before rebuilding. In a fresh clone the two builds completed in 26.21 seconds; a cached rerun completed in 0.01 seconds and both reported `ScriptingBridge`/`MediaRemote`.
...
## [2026-08-30] Task: todo-70
The Direct packaging script is now the sole local/CI release boundary: it always performs a fresh `NotchFlow (Direct)` build, preserves the Direct entitlements while applying hardened-runtime signing, lays out the app and `/Applications` symlink in a UDZO image, and names the artifact from the built app's `CFBundleShortVersionString`. With no membership credentials it completes through DMG and checksum creation while reporting Developer ID signing, notarization, and stapling as explicit skips; with the paired identity/profile variables it signs and notarizes both the app and final DMG. The release workflow calls this script and only owns ephemeral certificate/keychain credential setup, preventing packaging drift.
