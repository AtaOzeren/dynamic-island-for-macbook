...
///
/// Both are pure on purpose: the wiring in `KerNotchApp` reads a file and puts
...
struct WatchdogLaunchNoticeTests {
    ...
    func alertNamesTheReport() {
        let notice = WatchdogLaunchNotice(
            event: event(.quit, reportPath: "/tmp/kernotch-runaway.txt")
        )
        let alert = notice?.alert(isNotificationAuthorized: true)
        #expect(alert?.message.contains("/tmp/kernotch-runaway.txt") == true)
    }
