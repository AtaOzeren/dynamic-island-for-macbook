...
struct ClaudeCodeHookInstallerTests {
    ...
    private static let backupURL = homeDirectory.appending(
        path: ".claude/settings.json.kernotch-backup"
    )
    ...
    func uninstallStripsLegacyHookBesideBackup() throws {
        let original = Data(#"{"theme":"dark"}"#.utf8)
        let legacyCommand = #"python3 -c 'print(1)' # kernotch_hook_v3"#
        ...
        #expect(!remaining.contains("kernotch_hook_v3"))
        #expect(fileSystem.data(at: Self.backupURL) == original)
    }
                ...
                "Stop" : [
                  {
                    "hooks" : [
                      {
                        "type" : "command",
                        "command" : "URL=$(python3 -c 'uuid.uuid5(NS, x); print(1)' $EVENT); open -g kernotch://ai-status"
                      }
                    ]
                  }
                ]
    ...
    func replacesV2HooksForSubagentUpgrade() throws {
        let oldCommand = #"python3 -c 'print(1)' # kernotch_hook_v2"#
    ...
    func replacesV3Hooks() throws {
        let oldCommand = #"python3 -c 'print(1)' # kernotch_hook_v3"#
        ...
        let installed = try #require(fileSystem.text(at: Self.settingsURL))
        #expect(installed.contains(HookSnippetGenerator.managedHookMarker))
        #expect(!installed.contains("kernotch_hook_v3"))
    }
                ...
                "PreCompact" : [
                  {
                    "hooks" : [
                      {
                        "type" : "command",
                        "command" : "python3 -c 'uuid.uuid5(NS, x)'; open -g kernotch://ai-status?payload=x"
                      }
                    ]
                  }
                ]
    ...
    /// A hand-written hook that happens to open a `kernotch://` URL is the
    /// user's, not ours. Claiming it on the URL alone would delete their work
    /// on upgrade.
    @Test("install leaves a hand-written kernotch hook alone")
    func installPreservesHandWrittenKernotchHooks() throws {
        let handWritten = """
            {
              "hooks" : {
                "Stop" : [
                  {
                    "hooks" : [
                      {
                        "type" : "command",
                        "command" : "open -g kernotch://ai-status?payload=mine"
                      }
                    ]
                  }
                ]
    ...
    /// hook KerNotch added and leave the user's own hook on the shared event
    /// working.
