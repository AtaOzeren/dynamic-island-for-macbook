...
struct HookGenerationTests {
    @Test("generated agent hooks never launch KerNotch")
    func generatedHooksUseLoopbackOnly() {
        ...
        for hook in generatedHooks {
            ...
            #expect(!hook.contains("kernotch://ai-status"))
        }
    }

    @Test("generated hook markers identify loopback-only generations")
    func generatedHookMarkersAreCurrent() {
        #expect(HookSnippetGenerator.managedHookMarker == "kernotch_hook_v5")
        #expect(
        ...
        #expect(HookSnippetGenerator.codexNotifyMarker == "kernotch_codex_notify_v5")
        #expect(HookSnippetGenerator.codexLifecycleHookMarker == "kernotch_codex_hook_v4")
        #expect(
            HookSnippetGenerator.previousCodexLifecycleHookMarkers == [
                "kernotch_codex_hook_v1",
                "kernotch_codex_hook_v2",
                "kernotch_codex_hook_v3",
                "notchflow_codex_hook_v1",
                "notchflow_codex_hook_v2",
                "notchflow_codex_hook_v3",
            ]
        )
        #expect(
            HookSnippetGenerator.previousCodexNotifyMarkers == [
                "kernotch_codex_notify_v4",
                "notchflow_codex_notify_v4",
            ]
        )
    }

    @Test("the pre-rename scheme is matched but never emitted")
    func legacySchemeIsMatchedNeverEmitted() {
        #expect(HookSnippetGenerator.urlScheme == "kernotch")
        #expect(HookSnippetGenerator.legacyURLScheme == "notchflow")
        ...
        for text in generated {
            #expect(!text.contains("notchflow"))
        }
    }
    ...
    /// launch KerNotch after the user quit it, so its absence is part of the
    ...
    func hooksUseLoopbackWithoutALaunchFallback() throws {
        ...
        #expect(!command.contains("kernotch://ai-status"))
    ...
    func onlyPreToolUseCarriesAToolName() throws {
        ...
        #expect(try hookCommand(for: "PreToolUse", in: hooks).contains("        kernotch_tool_name(event),"))
        #expect(try hookCommand(for: "Stop", in: hooks).contains("        None,"))
        #expect(try !hookCommand(for: "Stop", in: hooks).contains("        kernotch_tool_name(event),"))
    }
    ...
    func hooksResolveTheirInterpreter() throws {
        ...
        for event in hooks.keys {
            ...
            #expect(command.contains("KERNOTCH_PY"), "\(event) does not resolve an interpreter")
        ...
        #expect(codexCommand.contains("KERNOTCH_PY"))
    ...
    func missingInterpreterExitsQuietly() throws {
        ...
        #expect(command.contains(#"[ -n "$KERNOTCH_PY" ] || exit 0"#))
    }
    ...
    private static func runHook(command: String, input: [String: Any]) throws -> [String: Any] {
        let temporaryDirectory = FileManager.default.temporaryDirectory.appending(
            path: "kernotch-claude-subagent-\(UUID().uuidString)",
        ...
        let discoveryURL = temporaryDirectory.appending(
            path: "Library/Application Support/KerNotch/ipc-port"
        )
        try FileManager.default.createDirectory(
