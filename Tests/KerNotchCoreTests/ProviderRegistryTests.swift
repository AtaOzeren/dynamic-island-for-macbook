...
struct ProviderRegistryTests {
    ...
    func doesNotReEndFinishedActivity() {
        ...
        let identity = ActivityIdentity("kernotch.music")
    ...
    func dropsEmissionFromDisabledProvider() {
        ...
        inFlight(.active(StubProviderActivity(identity: ActivityIdentity("kernotch.charging"))))
    ...
    func reEnablingRestartsProvider() {
        ...
        provider.emit(.active(StubProviderActivity(identity: ActivityIdentity("kernotch.music"))))

        #expect(provider.startCount == 1)
        #expect(registry.observingIdentifiers == [.music])
        #expect(receivedIdentities == [ActivityIdentity("kernotch.music")])
    }
