...
struct IPCURLParserTests {
    ...
    func uppercaseScheme() throws {
        let message = try IPCURLParser().parse(url(payload: payload(), scheme: "KERNOTCH"))
    ...
    func missingPayload() {
        expectParserError(.missingPayload, parsing: URL(string: "kernotch://ai-status")!)
    }
    ...
    func nonUTF8Payload() {
        expectParserError(
            .undecodablePayload,
            parsing: URL(string: "kernotch://ai-status?payload=%FF")!
        )
    }
    ...
    @Test(
        ...
        arguments: ["", "kernotch://", "://", "kernotch://ai-status?payload=%ZZ"]
    )
