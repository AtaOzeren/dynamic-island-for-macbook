...
///    timing. This layer is what makes the KerNotch side unified: whatever
...
/// later requires no KerNotch-side change.
@Suite("Agent parity")
    ...
    func toolInFlightIsReportedByEveryAgent() throws {
        ...
        for row in Self.parityTable {
            ...
            if row.state == .usingTool {
                #expect(
                    command?.contains("kernotch_tool_name(event),") == true,
                    "Claude Code \(row.claudeCodeTrigger) must pass the tool in flight (\(row.name))"
                )
            } else {
                #expect(
                    command?.contains("kernotch_tool_name(event),") == false,
