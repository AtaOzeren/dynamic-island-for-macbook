...
final class CPUWatchdogSupervisor: @unchecked Sendable {
    ...
    struct NotificationCopy: Sendable {
        ...
        static var standard: NotificationCopy {
            NotificationCopy(
                degrade: String(
                    localized: "KerNotch is using more CPU than expected. Animations are paused while it recovers."
                ),
                restart: String(
                    localized: "KerNotch is restarting because CPU stayed high despite protective measures. A diagnostic report was saved."
                ),
                quit: { reportPath in
                    String(
                        format: String(
                            localized: "KerNotch quit itself: CPU stayed high after 3 restarts within an hour. Diagnostic report: %@"
                        ),
                        reportPath ?? "—"
                    )
                }
    ...
    private static let deadlineQueue = DispatchQueue(
        label: "com.kernotch.cpu-watchdog.relaunch-deadline",
        qos: .utility
    )
